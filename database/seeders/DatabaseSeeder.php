...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $permissions = ['dashboard.view', 'stores.switch', 'products.view', 'products.create', 'products.update', 'products.delete', 'products.import', 'products.export', 'inventory.view', 'inventory.adjust', 'inventory.transfer', 'inventory.count', 'expenses.view', 'expenses.create', 'expenses.approve', 'sales.view', 'sales.create', 'sales.quote', 'sales.order', 'sales.update', 'sales.cancel', 'sales.return', 'sales.refund', 'sales.discount', 'sales.void', 'payments.create', 'payments.refund', 'purchases.view', 'purchases.create', 'purchases.approve', 'purchases.receive', 'customers.view', 'customers.create', 'customers.update', 'customers.delete', 'suppliers.view', 'suppliers.create', 'reports.view', 'reports.export', 'users.manage', 'roles.manage', 'settings.manage', 'audit.view'];
        foreach ($permissions as $slug) {
            DB::table('permissions')->updateOrInsert(['slug' => $slug], ['name' => Str::headline(str_replace('.', ' ', $slug)), 'module' => explode('.', $slug)[0], 'created_at' => $now, 'updated_at' => $now]);
        }
